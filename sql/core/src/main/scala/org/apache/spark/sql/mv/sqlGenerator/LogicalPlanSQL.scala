...
class LogicalPlanSQL(plan: LogicalPlan, dialect: SQLDialect) {
  ...
  def finalPlan(_plan: LogicalPlan): LogicalPlan = {
    ...
    val plan = _plan transformUp {
      case a@Join(l@Filter(lc, lchild), r@Filter(rc, rchild), joinType, condition, hint) =>
        Filter(And(lc, rc), Join(lchild, rchild, joinType, condition, hint))
      case a@Join(f@Filter(lc, lchild), r, joinType, condition, hint) => Filter(lc, Join(lchild, r, joinType, condition, hint))
      case a@Join(l, r@Filter(rc, rchild), joinType, condition, hint) => Filter(rc, Join(l, rchild, joinType, condition, hint))
    } transformDown {
...
//        case a@HadoopFsRelation(_, _, _, _, _, _) => dialect.relation3(a)
        case a@Aggregate(_, _, SubqueryAlias(_, LogicalRelation(_, _, _, _))) => dialect.relation3(a)
        case _ =>
      ...
      if (tableName != null) {
        ...
        dialect.subqueryAliasToSQL(identifier.qualifier.toString(), childSql)
      }
  ...
  def expressionToSQL(expression: Expression): String = expression match {
    ...
    */
    ...
    case replace@RegExpReplace(subject, regexp, rep, _) =>
  ...
  private def aggregateToSQL(a: Aggregate): String = {
    ...
    build(
      ...
      if (a.isInstanceOf[Aggregate]) s"(${logicalPlanToSQL(a.child)})" else logicalPlanToSQL(a.child),
