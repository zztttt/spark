...
import org.apache.spark.sql.catalyst.expressions.{Alias, And, AttributeReference, EqualNullSafe, EqualTo, Exists, ExprId, Expression, ListQuery, NamedLambdaVariable, PredicateHelper}
import org.apache.spark.sql.catalyst.expressions.VirtualColumn.groupingIdAttribute.exprId
import org.apache.spark.sql.catalyst.expressions.aggregate.AggregateExpression
import org.apache.spark.sql.catalyst.plans.logical.{Aggregate, Filter, Join, LogicalPlan, OneRowRelation, Project, Sample, SubqueryAlias}
import org.apache.spark.sql.execution.datasources.LogicalRelation
import org.apache.spark.sql.execution.{LogicalRDD, ScalarSubquery}
import org.apache.spark.sql.mv.TableHolder
import org.apache.spark.sql.mv.optimizer.rewrite.rule.{ProcessedComponent, RewriteContext}
...
//        m
  ...
  def extractFirstLevelJoin(plan: LogicalPlan) = {
    plan match {
      ...
      case p@Aggregate(_, _, Filter(_, join@Join(_, _, _, _, _))) => join
